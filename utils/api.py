...
what must be done in this class for the quiz to be generated is for the programmer to install langchain and
import the llm chain creating a chain that has generated questions and answers. the developer must create a
prompt template, then return the llm chain for the prompt 

def create_quiz():
template = " Create an exam of multiple choice questions with {number_of_questions} "
        f"questions and {number_of_answers} of possible answers in each question. "
        f"Put the correct answer in bold (surrounded by **) in its original spot. "
        f"The exam should be about {topics}. Only generate the questions and "
        f"answers, not the exam itself."
...
return prompt 
then create the chain and return it 
